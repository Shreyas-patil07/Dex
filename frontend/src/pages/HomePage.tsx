...
export const HomePage: React.FC = () => {
  ...
  return <div className="min-h-screen bg-transparent text-white">
    ...
    <main className="mx-auto max-w-7xl px-6 pb-20 pt-6">
      ...
      {!loading && !error && visible.length > 0 && <section className="mt-7 grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">{visible.map(item => { const title = item.title || item.name || 'Untitled'; const date = item.release_date || item.first_air_date || ''; return <article key={`${item.media_type || 'media'}-${item.id}`} className="group"><div className="overflow-hidden rounded-xl bg-white/[0.05]"><img src={`${TMDB_IMAGE}${item.poster_path}`} alt={title} loading="lazy" className="aspect-[2/3] w-full object-cover transition-transform duration-300 group-hover:scale-[1.025]" /></div><h2 className="mt-3 truncate text-sm font-semibold">{title}</h2><div className="mt-1 flex items-center gap-2 text-xs text-[#64748B]"><span>{item.media_type === 'tv' ? 'Series' : 'Movie'}</span>{date && <span>• {date.slice(0,4)}</span>}{typeof item.vote_average === 'number' && item.vote_average > 0 && <span>• {item.vote_average.toFixed(1)}</span>}</div></article>; })}</section>}
    </main>
