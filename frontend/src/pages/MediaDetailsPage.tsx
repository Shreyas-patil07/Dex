...
type Media = { id: number; title?: string; name?: string; poster_path?: string | null; backdrop_path?: string | null; release_date?: string; first_air_date?: string; runtime?: number | null; episode_run_time?: number[]; vote_average?: number; genres?: { name: string }[]; overview?: string; watch_providers?: { flatrate?: Provider[]; rent?: Provider[]; buy?: Provider[]; link?: string } };
...
export const MediaDetailsPage: React.FC = () => {
  ...
  useEffect(() => subscribeToAuthSession((currentUser) => { setUser(currentUser); setSignedIn(currentUser !== null); }), []);

  useEffect(() => {
    if (!mediaType || !tmdbId) { setError('Media not found.'); setLoading(false); return; }
    const controller = new AbortController();
    setLoading(true); setError(''); setItem(null);
    fetch(`${API_BASE}/api/media/${mediaType}/${tmdbId}`, { signal: controller.signal })
      .then(async response => { if (!response.ok) throw new Error('Could not load this title.'); return response.json(); })
  ...
  const addToList = async () => {
    ...
    if (!currentUser) { window.location.assign('/sign-up'); return; }
    setAdding(true); setListError('');
    try {
      const token = await currentUser.getIdToken();
      const response = await fetch(`${API_BASE}/api/watches`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ tmdb_id: Number(tmdbId), media_type: mediaType, title, status: 'want_to_watch' }),
      });
      const data = await response.json().catch(() => ({}));
      if (response.status === 409) { setAdded(true); return; }
  ...
  return <div className="min-h-screen bg-transparent text-white">
    <main className="mx-auto max-w-7xl px-6 pb-20 pt-8">
      ...
      <section className="mt-8 rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 sm:p-7">
