...
export const LandingPage: React.FC = () => {
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-[#080810] text-white">
      ...
      <style>{`
        .dex-orbit { transform-box: fill-box; transform-origin: center; }
        .dex-orbit-one { animation: dex-flow-one 14s linear infinite; }
        .dex-orbit-two { animation: dex-flow-two 18s linear infinite; }
        .dex-orbit-three { animation: dex-flow-three 22s linear infinite; }
        @keyframes dex-flow-one {
          0% { transform: translate3d(-180px, 80px, 0) rotate(-2deg); }
          50% { transform: translate3d(180px, -80px, 0) rotate(2deg); }
          100% { transform: translate3d(-180px, 80px, 0) rotate(-2deg); }
        }
        @keyframes dex-flow-two {
          0% { transform: translate3d(180px, -100px, 0) rotate(2deg); }
          50% { transform: translate3d(-180px, 100px, 0) rotate(-2deg); }
          100% { transform: translate3d(180px, -100px, 0) rotate(2deg); }
        }
        @keyframes dex-flow-three {
          0% { transform: translate3d(-120px, 100px, 0) rotate(-1.5deg); }
          50% { transform: translate3d(120px, -100px, 0) rotate(1.5deg); }
          100% { transform: translate3d(-120px, 100px, 0) rotate(-1.5deg); }
        }
        @media (prefers-reduced-motion: reduce) { .dex-orbit { animation: none; } }
      `}</style>
      ...
      <main className="relative z-10">
        <section className="flex min-h-[calc(100vh-92px)] flex-col items-center justify-center px-6 text-center">
          <h1 className="max-w-4xl text-4xl font-display font-extrabold leading-[1.05] tracking-tight sm:text-6xl lg:text-7xl">
            Welcome to your{' '}
        ...
        <section className="border-y border-white/[0.06] bg-white/[0.015] px-6 py-24 sm:py-28">
          <div className="mx-auto max-w-6xl">
            <div className="mb-12 max-w-xl">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-[#A855F7]">How Dex works</p>
              <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">Watch. Learn. Discover.</h2>
            </div>
            <div className="grid gap-px overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.07] sm:grid-cols-2">
              {features.map((feature, index) => (
                <article key={feature.title} className="bg-[#0B0B14] p-7 sm:p-9">
                  <span className="text-xs font-mono text-[#64748B]">0{index + 1}</span>
                  <h3 className="mt-5 text-xl font-semibold">{feature.title}</h3>
                  <p className="mt-3 max-w-md text-sm leading-6 text-[#94A3B8]">{feature.text}</p>
                </article>
              ))}
            </div>
          </div>
