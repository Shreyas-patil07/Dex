...
import { EmailAuthProvider, GoogleAuthProvider, linkWithCredential, linkWithPopup, onAuthStateChanged, signOut, User as FirebaseUser } from 'firebase/auth';
...
async function authorizedRequest(firebaseUser: FirebaseUser, path: string, options: RequestInit = {}) {
...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const syncResponse = await authorizedRequest(firebaseUser, '/api/auth/sync', { method: 'POST', body: JSON.stringify({ display_name: firebaseUser.displayName || undefined }) });
        if (!syncResponse.ok) { const data = await syncResponse.json().catch(() => ({})); throw new Error(data.detail || 'Could not synchronize your Dex account.'); }
        const [userResponse, watchesResponse] = await Promise.all([authorizedRequest(firebaseUser, '/api/me'), authorizedRequest(firebaseUser, '/api/watches')]);
        if (!userResponse.ok) { const data = await userResponse.json().catch(() => ({})); throw new Error(data.detail || 'Could not load your profile.'); }
        if (active) { setUser(await userResponse.json()); setWatches(watchesResponse.ok ? await watchesResponse.json() : []); }
  ...
  const handleGoogleLink = async () => {
    ...
    try {
      ...
      setLinkMessage('Google is now connected to this Dex account.');
    } catch (error) {
      if (error instanceof FirebaseError && error.code === 'auth/credential-already-in-use') setLinkMessage('That Google account is already connected to another account.');
      else if (error instanceof FirebaseError && error.code === 'auth/popup-closed-by-user') setLinkMessage('Google linking was cancelled.');
      else setLinkMessage(error instanceof Error ? error.message : 'Could not connect Google.');
    } finally { setLinking(null); }
  };

  const handlePasswordLink = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!firebaseUser) return;
    if (password.length < 8) { setLinkMessage('Password must be at least 8 characters.'); return; }
    if (password !== confirmPassword) { setLinkMessage('Passwords do not match.'); return; }
    if (!firebaseUser.email) { setLinkMessage('This account does not have an email address.'); return; }
    ...
    try {
      ...
      setLinkMessage('Email & Password is now connected to this Dex account.');
    } catch (error) {
      if (error instanceof FirebaseError && error.code === 'auth/provider-already-linked') setLinkMessage('Email & Password is already connected.');
      else if (error instanceof FirebaseError && error.code === 'auth/credential-already-in-use') setLinkMessage('This email already belongs to another Firebase account.');
      else setLinkMessage(error instanceof Error ? error.message : 'Could not connect Email & Password.');
    } finally { setLinking(null); }
  };

  const handleSignOut = async () => { setSigningOut(true); try { await signOut(auth); window.location.href = '/'; } finally { setSigningOut(false); } };
  ...
  return <div className="min-h-screen bg-[#080810] text-white">
    ...
    <main className="mx-auto max-w-5xl px-6 pb-20 pt-10">
      ...
      <section className="mt-12 rounded-3xl border border-white/[0.07] bg-white/[0.02] p-7 sm:p-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A855F7]">Account</p><h2 className="mt-2 text-2xl font-bold">Sign-in methods</h2><p className="mt-2 text-sm leading-6 text-[#94A3B8]">Connect multiple sign-in methods to the same Dex account.</p><div className="mt-6 space-y-3">
        <div className="flex flex-col gap-4 rounded-2xl border border-white/[0.07] bg-[#0B0B14] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Google</p><p className="mt-1 text-xs text-[#64748B]">{hasGoogle ? 'Connected to this account' : 'Not connected'}</p></div>{hasGoogle ? <span className="text-sm text-emerald-400">✓ Connected</span> : <button onClick={handleGoogleLink} disabled={linking !== null} className="rounded-xl bg-white/[0.06] px-4 py-2 text-sm font-semibold hover:bg-white/[0.09] disabled:opacity-50">{linking === 'google' ? 'Connecting…' : 'Connect Google'}</button>}</div>
        <div className="rounded-2xl border border-white/[0.07] bg-[#0B0B14] p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Email & Password</p><p className="mt-1 text-xs text-[#64748B]">{hasPassword ? `Connected as ${firebaseUser.email}` : `Use ${firebaseUser.email || 'your email'} to sign in`}</p></div>{hasPassword ? <span className="text-sm text-emerald-400">✓ Connected</span> : null}</div>{!hasPassword && <form onSubmit={handlePasswordLink} className="mt-5 grid gap-3 sm:grid-cols-2"><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="New password" minLength={8} required className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm outline-none placeholder:text-[#64748B] focus:border-[#7C3AED]" /><input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Confirm password" minLength={8} required className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm outline-none placeholder:text-[#64748B] focus:border-[#7C3AED]" /><button disabled={linking !== null} className="rounded-xl bg-[#7C3AED] px-4 py-3 text-sm font-semibold sm:col-span-2 disabled:opacity-50">{linking === 'password' ? 'Connecting…' : 'Add password'}</button></form>}</div>
      </div>{linkMessage && <p className="mt-4 rounded-xl border border-white/[0.07] bg-white/[0.03] px-4 py-3 text-sm text-[#CBD5E1]">{linkMessage}</p>}</section>
