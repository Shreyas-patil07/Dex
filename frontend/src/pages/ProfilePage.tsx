  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Re-create the local Dex session/identity after a database reset or
        // on a fresh Firebase account before requesting protected resources.
        const syncResponse = await authorizedRequest(firebaseUser, '/api/auth/sync', {
          method: 'POST',
          body: JSON.stringify({
        ...
        if (!syncResponse.ok) {
          ...
          throw new Error(data.detail || 'Could not synchronize your Dex account.');
        }

        const [userResponse, watchesResponse] = await Promise.all([
          authorizedRequest(firebaseUser, '/api/me'),
          authorizedRequest(firebaseUser, '/api/watches'),
        ]);
